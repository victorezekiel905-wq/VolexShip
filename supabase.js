...
(() => {
  ...
  fetchShipmentByCode = async function(code) {
    ...
    if (window.__veloxshipRuntime?.dbReady) {
      try {
        const response = await apiFetch(`/track/${encodeURIComponent(trackingCode)}`);
        if (response?.shipment) return mergeShipmentIntoCache(normalizeShipment(response.shipment));
      } catch (error) {
        logDataError('Public tracking API lookup failed.', error);
      }
    }
