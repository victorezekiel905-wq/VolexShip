...
app.get('/api/shipments/lookup/:code', async (req, res) => {
  ...
  const { data, error } = await supabase.from(TABLE).select('*').eq('tracking_code', code).maybeSingle();
  if (error) { logDbError('Lookup shipment failed', error); return res.status(500).json({ error: error.message }); }
  if (!data) return res.status(404).json({ error: 'Shipment not found.' });
  const s = fromRow(data);
  res.json({ shipment: { ...s, customerEmail: '', customerName: '' }, mode: 'cloud' });
});
